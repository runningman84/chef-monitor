...
module Sensu::Extension
  class InfluxDB < Handler
    ...
    def run(event_data)
      ...
      # This will merge : default conf tags < client embedded tags < check embedded tags < sensu client/host tag
      tags = @influx_conf['tags'].merge(event[:client][:influxdb][:tags]).merge(event[:check][:influxdb][:tags]).merge({'host' => client}).merge({'metric' => metric})

      if event[:check][:type] == 'metrics'
        ...
        event[:check][:output].split(/\n/).each do |line|
          ...
          key.gsub!(event[:client][:scheme_prefix], '')
